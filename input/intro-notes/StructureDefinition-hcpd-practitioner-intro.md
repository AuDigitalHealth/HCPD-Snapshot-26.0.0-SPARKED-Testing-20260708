...
* Practitioner: [Suppressed](StructureDefinition-suppressed.html)

#### Suppressed
The [extension:suppressed](StructureDefinition-suppressed.html) is used to indicate if this resource should be hidden from display in the Provider Directory. A Practitioner may elect to hide their visibility from those searching the Health Connect Provider Directory system. Presence of the extension indicates the resource is suppressed. When this extension is applied to a HCPD Practitioner resource, it's use is constrained to just the `practitioner-initiated` code. For more details, please see the extension intro notes.

### Usage notes
The following guidance maps key information requirements for HCPD Practitioner records to fields in the profile. Where a requirement references the HI Service as the authoritative source, publishers SHOULD prefer and validate values-such as practitioner identifiers, and names.
