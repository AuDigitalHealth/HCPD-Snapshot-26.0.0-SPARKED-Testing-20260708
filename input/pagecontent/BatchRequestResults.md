...
These NDJSON files may include resource instances of the usual resource types (for example `Location`, `Organization`, `Practitioner`, etc.). Bulk data extraction returns **all resources** regardless of active status or suppression. Implementers of the HCPD bulk data export **SHALL** respect the `suppressed` and `deactivated-by` extensions on returned resources, filtering these resources before displaying them to end consumers of the HCPD.
